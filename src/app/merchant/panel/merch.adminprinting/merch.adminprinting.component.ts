...
@Component({
  selector: 'app-merch.adminprinting',
  templateUrl: './merch.adminprinting.html',
  styleUrls: ['../merch.dashboard/merchdashboard.css',
...
export class MerchAdminprintingComponent {
  showNav = false
  merchData: any
  merchAdminDatas: any
  merchSelected = false
  ...
  merchantListDat(){
    this.curdService.getAllMerchant().subscribe((res: any) => {
      this.merchData = res
      this.merchData.forEach((element: any, index:any) => {
        if(this.devDatas.merchantid === element.merchdatas.merchantid)
        this.merchData = [element]
      });
    })
  }
